...
// BookingSearchrouter.get('/requests/:id',async(req,res)=>{
//     try{
...
//     }
// })
BookingSearchrouter.get('/search',async(req,res)=>{
...
BookingSearchrouter.post('/bookSlot',async(req,res)=>{
    try{ 
...
BookingSearchrouter.get('/requests/:id',async(req,res)=>{
    try{
        ...
        console.log("got the model", req.params);
        let requests = await ParkingModel.findById(req.params.id);
        console.log(requests);
        res.status(200).json({response:requests});
    }
    catch(error){
